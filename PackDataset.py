...
class processed_dataset(Dataset):
    def __init__(self, data, vocab):
        self.tokenized_data = [[vocab.stoi[word.lower()] for word in data_tuple[0].split(' ')] for data_tuple in tqdm(data)]
...
class processed_dataset_bert(Dataset):
    def __init__(self, data):
        ...
        for text, label in data:
            if type(text)!=str:
               print(text,label,count)
               continue
            count+=1
            t = torch.tensor(tokenizer.encode(text, truncation=True, max_length=256))
            self.texts.append(t)
